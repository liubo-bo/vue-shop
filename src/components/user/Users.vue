<template>
	<div>
		...
		<el-card>
			...
			<el-table :data="userlist" border stripe>
				...
				<el-table-column label="操作" width="180px">
					<template slot-scope="scope">
						...
						<!-- 分配角色按钮 -->
						<el-tooltip :enterable="false" class="item" placement="top" effect="dark" content="分配角色">
							<el-button size="mini" type="warning" 
						 	icon="el-icon-setting" @click="setRole(scope.row)"></el-button>
						</el-tooltip>
			...
			"handleCurrentChange" :current-page="queryInfo.pagenum" :page-sizes=
			"[1, 2, 5, 10]" :page-size="queryInfo.pagesize" layout="total, 
		...
		<!-- 修改用户的对话框 -->
		<el-dialog @close="editDialogClosed" title="修改用户" 
...
export default {
	...
	methods:{
		async getUserList() {
			const { data: res } = await this.$http.get('users', { 
				...
				if(res.meta.status !== 200) {
					return this.$message.error('获取管路员列表失败')
				}
		...
		async removeUserById(id) {
			//弹框提示用户是否删除
			const confirmRedult =  await this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
					...
					//如果用户点击确认，返回字符串confirm
					//如果用户点击取消，返回字符串cancel
					//console.log(confirmRedult)
